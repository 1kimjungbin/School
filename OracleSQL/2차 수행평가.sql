...
--     단, SCOTT은 검색 결과에서 제외하시오.
    ...
    where e.sal = (select sal from emp where lower(ename) = 'scott')
    and lower(e.ename) not in ('scott');


-- 12. CLARK 보다 급여가 많은 사원들의 사원번호, 사원이름, 급여를 검색하시오.
    select empno, ename, sal
    from emp
    where sal > (select sal from emp where lower(ename) = 'clark');

-- 13. JONES 보다 일찍 입사한 사원의 사원번호, 사원이름, 입사일을 검색하시오.
    select empno, ename, hiredate
    from emp
    where hiredate < (select hiredate from emp where lower(ename) = 'jones');
...
-- 15. salgrade가 2등급인 사원들의 평균 급여보다 적게 받는 사원들의 모든 정보를 검색하시오.

    select *
    from emp
    where sal < (select round(avg(sal),0) from emp e, salgrade s 
    where sal between s.losal and s.hisal and grade = 2) ;
...
-- 2. 급여가 2500 이상이고, 업무(job)가 MANAGER인 사원에 대하여 사원번호, 사원이름, 업무, 급여, 부서명을 검색하시오.
    ...
    from emp e join dept d
    using(deptno)
    where e.sal >= 2500 and lower(e.job) = 'manager';
...
-- 7. 커미션과 급여의 합이 1,600 이상인 사원의 사원이름, 부서명, 근무지, 연봉(sal), 커미션을 출력하시오.
    select e.ename, d.dname, d.loc, e.sal, e.comm, e.sal + e.comm
    from emp e join dept d
    using(deptno)
    where nvl(e.comm,0) + e.sal > 1600;
...
-- 15. salgrade가 2등급인 사원들의 평균 급여보다 적게 받는 사원들의 모든 정보를 검색하시오.
    select *
    from emp
    where sal < (select round(avg(sal),0) from emp e, salgrade s where sal between s.losal and s.hisal and s.grade = 2);
